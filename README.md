...
| [07. 화면](docs/07-screens.md) | **화면 27개**(디자인 원본 23개 + 나중에 추가된 비밀번호 찾기·정기 지출 관리 등)의 ID·이름·관련 R·F·진입 경로·다음 화면, 화면별 상세, **하단 탭 구조와 화면 전환 방식**(탭은 120ms 크로스페이드, 스택은 280ms 세로 슬라이드). 마지막에 **"문서엔 있는데 디자인엔 없는 화면"** 목록을 따로 남겼다 | 271줄 |
