...
| [04. 기능](docs/04-features.md) | **F1~F23**. R을 기능으로 옮긴 것으로, 기능마다 어느 R에서 나왔는지 출처를 달았다. **중요도 × 난이도 2축**으로 우선순위를 다시 매겼고, 에이전트가 던진 발산 질문의 채택·거절 결과와 MVP 이후 추가된 기능도 여기 있다 | 112줄 |
| [05. 정책](docs/05-policy.md) | **P1~P15**. "누가 / 언제 / 무엇을 할 수 없다 / 위반하면 화면에 뭐가 보이는가" 네 칸으로 쓴 규칙표. **상태값 2개**(그룹장 위임, 지출 인식 상태)와 **서비스 정책 SP1~SP4**(범위 밖 명시). 이 표가 그대로 Supabase **RLS 정책**이 됐다 | 67줄 |
| [06. 데이터](docs/06-data.md) | **엔티티 16개**(E1~E9·E12~E18 — E10 WeeklySettlement·E11 Budget은 v2에서 폐기되고 E13·E12로 대체됐다)와 필드 전부, 엔티티 간 **관계**, 상태값이 어느 필드와 연결되는지. 나중에 추가된 것(정기 지출 E16, 그룹 예산 E17, 그룹 밥주기 E18)도 언제·왜 생겼는지 적어 뒀다 | 289줄 |
| [07. 화면](docs/07-screens.md) | **화면 27개**(디자인 원본 23개 + 나중에 추가된 비밀번호 찾기·정기 지출 관리 등)의 ID·이름·관련 R·F·진입 경로·다음 화면, 화면별 상세, **하단 탭 구조와 화면 전환 방식**(탭은 120ms 크로스페이드, 스택은 280ms 세로 슬라이드). 마지막에 **"문서엔 있는데 디자인엔 없는 화면"** 목록을 따로 남겼다 | 257줄 |
| [PRD (종합)](docs/PRD.md) | 위 문서를 한 곳에 요약 — 제품 개요, 핵심 사용자 흐름, **MVP/v2 기능 범위**, 화면·데이터·정책 요약, **기술 스택**, **7일 마일스톤**, 그리고 **열린 이슈를 착수 우선순위로 재배열한 목록** | 194줄 |
| [저금통 펫 명세](docs/08-pet-feature-spec.md) | 펫 기능만 따로 뗀 화면·기능 명세. 전체 개념, 펫 선택·상세·먹이주기 팝업·절약 리포트·배지·그룹 랭킹, 마이페이지 변경점, **백엔드 설계 시 참고사항**, 그리고 확인 후 결정된 것과 **여전히 남은 `[?]`** 정리 | 120줄 |

- 3인 개인 초안: [docs/drafts/](docs/drafts/) (01~03을 각자 한 벌씩, 총 3벌) · [초안 비교 정리본](docs/논의-아티팩트-정리본.md) — 세 초안의 차이를 나란히 놓고 무엇을 채택했는지 기록

> **문서와 실제가 갈라진 곳** — PRD 7절의 기술 스택은 `React + Vite`로 적혀 있지만 실제로는 **Next.js**로 만들었다. 셋업 단계에서 바꾼 결정인데 PRD에 반영하지 않았다. 기능·화면·정책 문서는 구현 중에도 계속 갱신했지만(9/21에도 정기 지출·그룹 예산을 반영), 기술 스택처럼 "한 번 정하고 안 보는 칸"은 그대로 남았다.
...
**로그인 화면** — 아이콘 타일·제목·입력칸 radius·"로그인 상태 유지" 토글까지 그대로 옮겼다. 구현에서 더해진 것은 `비밀번호를 잊으셨나요?`와 **카카오로 계속하기** 버튼 둘로, 기획 확정 뒤에 추가된 기능이라 디자인 원본에는 없다.

**홈 화면** — 인사말·수입 카드·보라색 지출 카드·점선 "그룹 만들기"·하단 탭까지 원본 구조를 지켰다. 더해진 것은 `이번 달 목표 설정하기` 카드와 카테고리 **도넛 차트**(원본은 가로 막대), 빠진 것은 원본 아래쪽 월 캘린더다 — 캘린더는 별도 화면으로 옮겼다.
...
3. 화면 동작은 로컬 HTTPS 서버(`npm run dev:https`)를 띄우고 실제 계정으로 로그인해 확인한다. 서버 로그(`[account]`, `[receipt-ocr]` 등)에 원인 진단용 로그를 남겨 실패 지점을 특정한다. 또는 vercel의 각각 브랜치 도메인 링크를 통해서 직접 확인.
