...
| Claude Design — 2b. 메인 (홈) | 실제 홈 (로그인 후) |
|---|---|
| ![Claude Design 홈](docs/images/cdesign-home.png) | ![실제 홈 화면](docs/images/real-home.png) |

인사말·수입 카드·보라색 지출 카드·카테고리 비중 바·점선 "그룹 만들기"·하단 탭까지 원본 구조를 그대로 지켰다. 구현에서 **더해진 것**은 `이번 달 목표 설정하기` 카드(월별 목표 기능이 나중에 들어왔다)이고, **빠진 것**은 원본 아래쪽의 월 캘린더다 — 캘린더는 별도 화면으로 옮겼다. 카테고리 비중(원본 42/28/17%)은 고정값이 아니라 실제 지출에서 계산하므로, 식비만 입력된 계정에서는 100%로 나온다.

캐릭터 색 커스터마이즈 화면은 이렇다.

![캐릭터 커스텀 화면](docs/images/character-custom.png)

원본 PNG 한 장에 **부위 마스크를 합성해** 몸·가방·눈·잎사귀 색을 바꾼다. 코인은 §11 규칙대로 커스터마이즈 대상에서 빠져 **원래 금색**을 유지하고, 성장 단계(알·유년기·청소년기·성체)는 도달한 범위 안에서 골라 볼 수 있다.
...
- **기능이 다 된 뒤 2차 점검을 따로 돌린 것.** "동작한다"에서 끝내지 않고 완성된 코드를 한 번 더 훑는 단계를 뒀더니, 개별로는 잘 돌아가 보이던 날짜/시간대 버그 3건이 한꺼번에 나왔다.
...
| **타임존 버그가 1차 점검을 통과했다** — 출석 연속일수·저녁 리마인더·페이스 경고가 각각 다른 증상으로 어긋났다 | "오늘 날짜"를 **모듈을 불러올 때 한 번만 고정**하는 패턴이 세 곳에 복사돼 있었다. 증상이 제각각이라 처음엔 서로 다른 버그로 보였다 | 기능이 끝난 뒤 2차 점검을 따로 돌려 3건을 한 번에 찾았고, 날짜를 쓸 때마다 다시 계산하도록 통일했다 |
...
2. **이슈·PR 템플릿을 처음부터 만들고, 화면마다 담당자를 한 명만 둔다.** 브랜치명만으로는 누가 무엇을 하는지 겹치는 걸 못 막았고, 실제로 같은 입력칸을 세 사람이 같은 시각에 고쳤다.
