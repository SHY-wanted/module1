...
1. `npx tsc --noEmit` → `npx eslint <변경 파일>` → `npx next build --webpack` 3단계를 **매 변경마다** 통과시킨다.
   (npx tsc --noEmit — 타입 검사만 하는 명령. --noEmit은 "실제 파일은 만들지 말고 검사만 해라"는 뜻. 예를 들어 숫자가 들어가야 할 자리에 문자를 넣었다든지, 존재하지 않는 값을 참조했다든지 하는 "타입이 안 맞는 실수"를 여기서 잡는다.

npx eslint <변경 파일> — 코드 스타일·흔한 버그 패턴을 검사하는 도구. 안 쓰는 변수를 만들어놨거나, React에서 자주 실수하는 패턴(예: 의존성 빠뜨림) 같은 걸 잡아줌. <변경 파일> 자리에는 실제로 고친 파일 경로를 넣는다.

npx next build --webpack — 실제로 배포할 수 있는 상태로 통째로 빌드해보기. 1·2번은 통과해도 빌드 자체가 안 되는 경우가 있어서(예: 서버에서만 써야 할 코드를 클라이언트에 잘못 섞었다든지), 마지막에 진짜 배포 가능한지 확인하는 단계.
