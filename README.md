...
**MVP를 끝낸 뒤 더 붙인 것** (9/19~9/22, PR #17~#23과 main 직접 커밋)
...
| **보안 강화** | REST/RPC를 직접 호출해 코인·XP를 무한정 채우거나 아무 그룹의 그룹장을 자가 임명할 수 있던 구멍을 막았다. 보상 계산을 클라이언트에서 서버 RPC로 옮기고, `pets`의 경제 컬럼은 컬럼 단위 권한으로 잠갔다 |
| 회원가입 비밀번호 검증 | 입력칸에 "8자 이상"이라고 써 있는데 실제로 검사하는 코드가 없어 7자로도 가입됐다 — 이메일 형식과 같은 인라인 오류로 막았다 |
...
- **폴더 구조**: `app/`(라우트·API) · `components/screens/`(화면 컴포넌트 43개) · `lib/`(store·selectors·순수 로직) · `docs/`(기획) · `supabase/`(SQL 32개) · `scripts/`(마스크 생성·검증) · `.claude/agents/`(에이전트·스킬 문서)
...
총 **PR 23개** (병합 19 · **열린 PR 없음**). 계정명은 `Lezo-Jong`=이종민, `hhy0123`=황하윤, `arim0317-dot`=신아림.
...
| #23 | 개인 펫 코인·XP 지급을 원자적 RPC로 통일, 목표 정산 버그 수정 | `hybran9` | 병합 |
...
- 총 커밋 **188개**, 머지 커밋 **37개**, PR **23개**(병합 19 · 열린 것 없음).
...
- Supabase: `supabase/schema.sql` → `002` ~ `032` 순서대로 SQL Editor에서 실행 (`fix_group_members_recursion.sql`은 `schema.sql`만 실행해 둔 기존 프로젝트를 고치는 용도라 새로 세팅할 땐 건너뛴다. `033`은 시연 전용이라 아래 별도 설명 참고)
...
>
> **`025`~`031`은 보안 강화**(REST/RPC 직접 호출로 코인·XP·그룹 권한을 조작할 수 있던 구멍 차단), **`032`는 그 과정에서 생긴 출석체크 버그 수정**이다 — `032`를 빠뜨리면 "출석하기"를 눌러도 코인이 들어오지 않는다.
...
**계정 관리 스크립트** — [scripts/seed-instructor-accounts.mjs](scripts/seed-instructor-accounts.mjs). `.env.local`의 서비스 롤 키를 쓰므로 그 파일이 있는 PC에서만 동작한다.

| 명령 | 하는 일 |
|---|---|
| `node scripts/seed-instructor-accounts.mjs` | 계정·그룹·샘플 데이터 생성. 여러 번 실행해도 안전하고, 이미 있으면 비밀번호만 다시 맞춘다 |
| `… --demo-reset` | **리허설을 다시 돌릴 때.** 오늘의 출석체크·밥주기 기록을 지우고 코인을 100개로 채우고 온보딩 투어가 다시 뜨게 한다. 계정·그룹·지출은 건드리지 않는다 |
| `… --delete` | 계정·그룹·지출 전부 삭제(평가 종료 후 정리) |

> 돌린 뒤에는 브라우저를 **새로고침**해야 한다. 앱이 이 값들을 로그인·페이지 로드 시점에 읽기 때문에, 열려 있는 화면에는 바로 반영되지 않는다.

**시연용 예외 — 팀장 계정만 밥주기 무제한**

밥주기는 서버에서 하루 1회로 막혀 있다(`pet_feedings`의 `(pet_id, user_id, fed_date)` 유니크 제약). 그런데 발표에서 성장 4단계(알 → 유년기 → 청소년기 → 성체)를 그 자리에서 보여주려면 연속으로 먹여야 한다. 그래서 **팀장 계정 하나에만** 예외를 두는 SQL을 따로 준비했다.

| 파일 | 언제 실행 |
|---|---|
| [`033_demo_unlimited_feed.sql`](supabase/033_demo_unlimited_feed.sql) | 발표 전 — 예외 열기 |
| [`033_revert_demo_unlimited_feed.sql`](supabase/033_revert_demo_unlimited_feed.sql) | **발표 후 — 원상복구** |

- 유니크 제약은 **그대로 둔다.** 제약을 없애면 모든 사용자에게 구멍이 생기므로, `feed_pet()` 안에서 그 계정(uuid 하나)일 때만 오늘 기록을 지우고 다시 넣는다.
- 코인 차감(5)·XP 획득(15)·단계 승급은 **정상 경로와 동일**하다 — 시연에서 보이는 성장은 조작한 화면이 아니라 실제 로직을 탄 결과다. 팀장 계정 펫에는 코인 10,000개(= 2,000번 분량)를 넣어 뒀다.
- 이건 `025`~`031` 보안 강화가 막아둔 경제 조작에 예외를 **일부러 하나 여는 것**이다. 그래서 되돌리는 파일을 함께 뒀고, 평가가 끝나면 닫는 것을 전제로 한다.
...
| **화면에는 규칙이 써 있는데 검사하는 코드가 없었다** — 회원가입 비밀번호 칸에 "8자 이상"이라고 적혀 있었지만 7자로도 가입됐다 | 화면 문구를 먼저 만들고 검증은 나중에 붙이려다 잊었다. 서버(Supabase) 기본 최소 길이가 6자라 서버도 안 막았고, 화면 문구만 보고 "되는 줄" 알았다 | 이메일 형식 검사와 같은 인라인 오류로 막았다. 07-screens.md에 `[?]`로 남아 있던 자리를 그제서야 채운 셈이다 |
| **보안 수정이 멀쩡하던 기능을 조용히 깨뜨렸다** — 출석체크를 눌러도 코인이 안 들어왔다 | 보상 계산을 서버 RPC로 옮기면서 함수 반환 컬럼 이름(`checkin_date`)이 테이블 컬럼과 겹쳤다. 마이그레이션 **실행은 성공**하고 호출할 때만 실패해서, 적용 직후엔 아무도 몰랐다 | 테이블 참조에 별칭을 붙여 고쳤다(`032`). 이후 임시 계정으로 주요 기능 15개를 실제로 호출해 다른 곳은 멀쩡한지 확인했다 |
...
5. **보안 수정 뒤에는 멀쩡하던 기능부터 다시 눌러본다.** 이번에 보상 계산을 서버로 옮기면서 출석체크가 조용히 깨졌는데, 마이그레이션 실행은 성공해서 아무도 몰랐다. "고친 곳"이 아니라 "건드린 테이블을 쓰는 모든 화면"이 점검 범위다.
6. **화면에 규칙을 쓸 때 검증 코드를 같이 넣는다.** "8자 이상"이라는 문구만 있고 검사가 없어서 7자로 가입이 됐다. 문구는 약속이지 검증이 아니다.
7. **"여럿이 같이 쓴다"는 전제를 1일차 설계에 넣는다.** 공유 가계부를 만들면서 데이터는 로그인할 때 한 번만 읽는 구조로 짰다. 기능이 거의 끝난 뒤에야 "새로고침해야 서로가 보인다"는 게 드러나서, 막판에 13개 테이블을 실시간으로 바꿔야 했다. 혼자 쓰는 앱이면 안 겪었을 문제다.
