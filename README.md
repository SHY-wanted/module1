...
시연에 쓰는 실제 계정과 **분리된 테스트 계정**이다. 비밀번호는 공통 `ShooT2026!`.

| 계정 | 이메일 | 권한 | 이 계정으로 확인할 수 있는 것 |
|---|---|---|---|
| 강사-그룹장 | `instructor.owner@shoottest.com` | **그룹장(OWNER)** | 그룹 예산 설정·수정, 그룹 이름 변경, 그룹장 위임 |
| 강사-그룹원 | `instructor.member@shoottest.com` | **그룹원(MEMBER)** | 그룹 피드 조회, 본인 지출만 수정·삭제. 그룹 예산은 **읽기 전용** |
| 강사-일반사용자 | `instructor.solo@shoottest.com` | **그룹 미소속** | 그룹이 없을 때의 화면 — 공유 select가 아예 안 보임(P8) |
