// 참고 이미지(사용자 제공, design-assets/pet-mascot-reference.png — 5스테이지 구버전)를 부위별
// 레이어로 분리하는 스크립트. 2026-09-15에 사용자가 스테이지별 커스텀 항목까지 명시된 더 정확한
// 참고 이미지(design-assets/pet-mascot-reference-v2.png)를 새로 줘서, 알/유년기/청소년기/성년기
// 4단계는 extract-pet-layers-v2.js로 완전히 대체됐다(public/pets/v2-*.png).
//
// 이 스크립트는 이제 "시무룩(stage-3-sulking)" 하나만 계속 쓴다 — v2 참고 이미지에는 시무룩 그림이
// 없어서(성장 단계 4개만 있음), 구버전 이미지에서 추출한 걸 그대로 유지한다. 재실행해도 결과 파일 중
// stage-3-sulking-*.png만 실제로 쓰이고, 나머지(stage-1-egg-*·stage-2-*·stage-3-*·stage-4-*)는
// v2로 대체되어 코드에서 더 이상 참조하지 않는다 — 재실행 후 그 파일들은 지워도 된다.
//
// 이력(요약, 구버전 4단계까지 겪은 버그들 — 지금은 v2로 넘어갔지만 시무룩에도 같은 기법을 쓰므로
// 남겨둠): 1차 도형 마스크 → 2차 알 단계 눈·잎사귀 보정 → 3차 dest-in/out이 알파 채널로만 동작한다는
// 걸 몰라서 1채널 마스크가 무시되던 버그 수정 → 4차 눈동자도 색 임계값 방식으로 수정 → 5차 eyeBoxes·
// leaf 범위를 픽셀 스캔으로 재조정 → 6차 볼·입 위치도 색 임계값으로 교체.
...
const STAGES = {
  ...
  "stage-2": {
    ...
    // ry를 줄여 줄기가 실제로 몸통과 만나는 지점(y≈90, 중앙 열 색 스캔) 아래로 안 넘어가게 함 —
    // stage-3와 같은 "목에 이상한 띠" 버그가 여기도 있었다.
    leaf: [ellipse(138, 55, 80, 36)],
  ...
  "stage-3": {
    // 잎사귀 ry가 너무 커서(62) 머리 위쪽까지 잎사귀색으로 칠해져 목 주변에 어색한 띠가 생겼다
    // (사용자 신고, 2026-09-15). 줄기가 실제로 몸통과 만나는 지점(y≈112, 중앙 열 색 스캔으로 확인)
    // 바로 아래에서 끊기도록 줄였다. 가계부도 실제 위치보다 왼쪽·위로 치우쳐 있어서(그리드 오버레이로
    // 재확인) 좌표를 다시 잡았다.
    leaf: [ellipse(135, 68, 90, 48)],
    ledger: [rect(145, 255, 80, 75, 10)],
  ...
  "stage-4": {
    leaf: [ellipse(147, 78, 112, 45)], // 줄기-몸통 경계(y≈120, 중앙 열 색 스캔) 아래로 안 넘어가게
  ...
  "stage-3-sulking": {
    ...
    leaf: [ellipse(150, 85, 85, 52)], // 줄기-몸통 경계(y≈140) 아래로 안 넘어가게
