  ...
  return (
    <div style={{ height: "100%", width: "100%", boxSizing: "border-box", background: "var(--shoot-bg)", display: "flex", flexDirection: "column", position: "relative" }}>
      <div style={{ padding: "20px 20px 12px", flexShrink: 0, display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 800, color: "var(--shoot-text)" }}>내 그룹 관리</div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 20px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {myGroups.map((g) => {
          ...
          return (
            <div key={g.id} style={{ background: "var(--shoot-surface)", borderRadius: 18, border: "1px solid var(--shoot-border)", padding: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 800, color: "var(--shoot-text)" }}>{g.name}</div>
                  <span
                    style={{
                      fontSize: 11,
                      fontWeight: 800,
                      color: isOwner ? "var(--shoot-accent)" : "var(--shoot-text-muted)",
                      background: isOwner ? "var(--shoot-surface-alt)" : "var(--shoot-divider)",
                      padding: "2px 8px",
                      borderRadius: 8,
                    }}
                  >
              ...
              <div style={{ height: 1, background: "var(--shoot-divider)", margin: "4px 0 12px" }} />
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {isOwner && (
                  // 07-screens.md "10a '그룹장 위임' → 10a-1" — 2026-09-11 팀 결정(신규 화면).
                  <div
                    onClick={() => nav.push({ id: "delegateSelect", groupId: g.id })}
                    style={{ height: 42, borderRadius: 12, background: "var(--shoot-surface)", border: "1.5px solid var(--shoot-border)", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontSize: 13, fontWeight: 700, color: "var(--shoot-text)", cursor: "pointer" }}
                  >
                    <UsersIcon size={15} color="var(--shoot-text)" />
                    그룹장 위임
                  </div>
                )}
                <div
                  ...
                  style={{ height: 42, borderRadius: 12, background: "var(--shoot-surface)", border: "1.5px solid #E8B4B4", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontSize: 13, fontWeight: 700, color: "#B23B3B", cursor: "pointer" }}
                >
        ...
        {myGroups.length === 0 && (
          <div style={{ textAlign: "center", color: "var(--shoot-text-muted)", fontSize: 13, fontWeight: 600, padding: "20px 0" }}>아직 속한 그룹이 없어요</div>
        )}
      </div>
      ...
      {confirmingGroupId && (
        <div style={{ position: "absolute", inset: 0, background: "rgba(45,42,62,0.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: 32, zIndex: 20 }}>
          <div style={{ background: "var(--shoot-surface)", borderRadius: 20, padding: 22, width: "100%", textAlign: "center" }}>
            <div style={{ fontSize: 16, fontWeight: 800, color: "var(--shoot-text)" }}>정말 나가시겠어요?</div>
            <div style={{ fontSize: 12, color: "var(--shoot-text-muted)", marginTop: 8, fontWeight: 600, lineHeight: 1.5 }}>
