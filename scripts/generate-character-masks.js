 ...
 * 실측 — 흰자 0~7, 눈동자 58~66, 몸통 라벤더 50~55. 20이면 이 셋은 넉넉히 갈린다.
 *
 * 그런데 눈동자 안의 하이라이트(반짝이는 점)는 사정이 다르다. 하이라이트 자체는 무채색 0~3인데,
 * 그 둘레에 눈동자로 흐려지는 안티에일리어싱 띠가 있고 이 띠는 무채색 8~23까지 걸쳐 있다(실측).
 * 20으로 자르면 이 띠 대부분이 "하이라이트"로 잡혀 원본 그대로 남는데, 띠의 실제 색은 순백이
 * 아니라 연보라(밝기 71~89%)라서, 눈을 진한 색(검정 등)으로 바꾸면 하이라이트 옆에 길게 늘어진
 * 회색-보라색 꼬리처럼 두드러져 보였다(사용자 신고: "사진 올려준 부분 이어줘라" — 하이라이트
 * 마스크 구멍이 눈 흰자 쪽으로 갈라진 것처럼 보이는 문제).
 *
 * 그래서 10으로 낮췄다 — 흰자(0~7)는 여전히 넉넉히 잡히고, 하이라이트의 진짜 밝은 중심(0~3)만
 * 남고 흐려지는 꼬리(8~23)는 더 이상 "무조건 원본 유지"에 안 걸린다. 그 꼬리는 원래 있던
 * eyeSoftWeight(부드러운 경계 블렌딩)로 넘어가거나 눈 마스크에 포함되어, 눈 색을 바꾸면 그 색을
 * 따라간다 — 늘어진 꼬리 대신 깔끔하게 이어진 하나의 눈으로 보인다.
 */
const EYE_WHITE_CHROMA = 10;
